...
route_registered_product_control() {
    ...
    case "$operation:$PREBOOTSTRAP_SUBCOMMAND" in
        ...
        merge:next|merge:resolve|merge:review|merge:reopen|merge:reconcile|merge:refresh|merge:drive|merge:withdraw|merge:arbiter|merge:recover-authority-drift|merge:supersede-lifecycle-journal) effective_operation=orchestration ;;
