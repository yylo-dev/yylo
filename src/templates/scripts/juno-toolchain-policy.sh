...
YYLO_LEDGER_COMPAT_RANGE='0.3.2'
...
if (major, minor, patch) != (0, 3, 2) or "rc" in output:
    print(f"unsupported yylo-ledger version {output.strip()}; required 0.3.2", file=sys.stderr)
    raise SystemExit(3)
print(f"{major}.{minor}.{patch}")
PY
}
