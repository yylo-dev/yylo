...
YYLO_LEDGER_COMPAT_RANGE='0.4.0'
...
if (major, minor, patch) != (0, 4, 0) or "rc" in output:
    print(f"unsupported yylo-ledger version {output.strip()}; required 0.4.0", file=sys.stderr)
    raise SystemExit(3)
print(f"{major}.{minor}.{patch}")
PY
}
