...
Activated consumer generations admit their complete controller-local runtime through the shared engine, using a tarball-authenticated captured import closure; they never rewrite historical product scripts/inventory or move its ref. Juno source targets retain exact source-runtime agreement. After activation, `scripts doctor` and `integration runtime-doctor` share generation admission rather than requiring obsolete target-bound receipts. Explicit target-bound `yy scripts update --force` remains separately admitted only with exact release/target/receipt/policy proof; it is not generic recovery and never edits tracked policy.
