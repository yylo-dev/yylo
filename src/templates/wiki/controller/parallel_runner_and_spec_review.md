...
**Cheap admission and composition boundary:** Before expensive work, the aggregate plan evaluates state, fencing, FIFO/dependencies, runtime/policy/package identity, blob origins, exact admission/parity, locks, routing, and evidence eligibility through `juno_path_origin_projection.v1`; target drift privately composes without source-tip rewrite and only genuine overlap conflicts.
